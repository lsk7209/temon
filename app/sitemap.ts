 ...
 * - 자동으로 스캔하여 사이트맵에 포함됨
 * 
 * 두 방법 모두 자동으로 동작하며, 파일 시스템 스캔이 우선입니다.
 * 
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // 테스트 라우트 동적 생성 (파일 시스템 기반으로 실제 존재하는 페이지만 추가)
  ...
  const sitemapEntries: MetadataRoute.Sitemap = [
    ...
    // 테스트 라우트 - 인트로 페이지만 포함 (결과 페이지 및 테스트 진행 페이지는 제외)
    ...testRoutes
      .filter(route => !route.path.includes('/test/result') && !route.path.includes('/test'))
      .map(route => ({
        url: route.path,
        lastModified: route.lastModified || new Date(),
        changeFrequency: route.changeFrequency || 'weekly' as const,
