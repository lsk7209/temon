...
export async function generateMetadata(
  ...
  return {
    ...
    openGraph: {
      ...
      description: desc,
      images: [
        `https://temon.kr/api/og?title=${encodeURIComponent(title)}&desc=${encodeURIComponent(data.typeDetail.typeCode)}`,
      ],
    },
  };
}
...
export default async function ResultPage({ params }: Props) {
  ...
  let traits: string[] = [];
  try {
    traits = JSON.parse(typeDetail.traits as string);
  } catch (e) {}

  let tips: string[] = [];
  try {
    tips = JSON.parse(typeDetail.tips as string);
  } catch (e) {}
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="text-center mb-8">
            <h1 className="text-xl md:text-2xl font-bold text-gray-500 mb-2">
              {test.title} 결과
            </h1>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-8">
            ...
            {tips.length > 0 && (
              <>
                <h3 className="text-xl font-bold text-gray-900 mb-6 mt-8 border-b pb-2">
                  💡 꿀팁
                </h3>
                <ul className="space-y-3">
                  {tips.map((tip, i) => (
