        ...
        <Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur">
          <CardContent className="p-8 space-y-6 text-center">
            <div className="space-y-2">
              <div className="text-sm text-muted-foreground">🫧 알고리즘 버블 유형 테스트 결과</div>
              <h1 className="text-3xl md:text-4xl font-bold">{label} ({type})</h1>
              <p className="text-muted-foreground">추천 피드 소비 습관으로 보는 취향 확장 vs 고정 성향에 강점이 있는 유형입니다.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-3 justify-center">
              <ShareButtons
                testId="algorithm-bubble-type"
                testPath="/tests/algorithm-bubble-type/test"
                resultType={type}
                resultId={resultId}
              ...
              <Button variant="outline" asChild>
                <Link href="/tests/algorithm-bubble-type/test"><RotateCcw className="h-4 w-4 mr-2" /> 다시 테스트</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
