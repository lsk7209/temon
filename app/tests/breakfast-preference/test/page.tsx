...
export default function BreakfastPreferenceTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/breakfast-preference/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/breakfast-preference/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("breakfast-preference")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("breakfast-preference", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-yellow-500 bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-950 dark:via-orange-950 dark:to-amber-950 shadow-lg"
