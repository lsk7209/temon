...
const questions = [
  {
    id: 1,
    q: "조리 방법 선택 빈도는?",
    a1: { text: "항상 다양한 조리 방법", tags: ["E"] },
    a2: { text: "가끔 조리 방법 변경", tags: ["I"] },
  },
  {
    id: 2,
    q: "조리 방법 선택 방식은?",
  ...
  },
  {
    id: 6,
    q: "조리 방법 패턴은?",
    a1: { text: "새로운 방법 시도", tags: ["N"] },
    a2: { text: "익숙한 방법 유지", tags: ["S"] },
  },
  {
    id: 7,
    q: "조리 방법 이유는?",
...
export default function CookingMethodTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/cooking-method/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/cooking-method/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("cooking-method")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("cooking-method", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-orange-500 bg-gradient-to-r from-orange-50 to-red-50 dark:from-orange-950 dark:via-red-950 dark:to-pink-950 shadow-lg"
