...
const questions = [
  {
    id: 1,
    q: "요리할 때 음악 듣기 빈도는?",
    a1: { text: "항상 음악 듣기", tags: ["E"] },
    a2: { text: "가끔 음악 듣기", tags: ["I"] },
  },
  {
    id: 2,
    q: "요리할 때 음악 선택 방식은?",
  ...
  },
  {
    id: 6,
    q: "요리할 때 음악 패턴은?",
    a1: { text: "새로운 음악 시도", tags: ["N"] },
    a2: { text: "익숙한 음악 유지", tags: ["S"] },
  },
  {
    id: 7,
    q: "요리할 때 음악 이유는?",
...
export default function CookingMusicTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/cooking-music/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/cooking-music/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("cooking-music")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("cooking-music", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pink-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-pink-500 bg-gradient-to-r from-pink-50 to-rose-50 dark:from-pink-950 dark:via-rose-950 dark:to-red-950 shadow-lg"
