...
export default function EveningSleepPrepTest() {
  ...
  useEffect(() => {
    trackTestStart("evening-sleep-prep")
  }, [])
  ...
  const handleChoiceSelect = async (tags: string[]) => {
    ...
    trackTestProgress("evening-sleep-prep", currentQuestion + 1, questions.length)
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      try {
        ...
        router.push(`/tests/evening-sleep-prep/test/result?type=${result}&id=${resultId}`)
      } catch (error) {
        console.error("결과 저장 실패:", error)
        router.push(`/tests/evening-sleep-prep/test/result?type=${result}`)
      }
    }
  }
