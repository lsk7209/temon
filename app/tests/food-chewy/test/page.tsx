...
const questions = [
  {
    id: 1,
    q: "식당에서 메뉴를 고르다가 '쫄깃한 떡볶이'와 '부드러운 라면' 중 선택할 때",
    a1: { text: "항상 쫄깃한 떡볶이를 선택한다", tags: ["E"] },
    a2: { text: "가끔 쫄깃한 떡볶이를 선택한다", tags: ["I"] },
  },
  {
    id: 2,
    q: "떡볶이를 먹다가 떡이 너무 쫄깃해서 턱이 아플 것 같을 때",
    a1: { text: "오히려 더 쫄깃한 맛을 좋아한다", tags: ["N"] },
    a2: { text: "적당한 쫄깃함을 선호한다", tags: ["S"] },
  },
  {
    id: 3,
    q: "식당에서 메뉴판을 보다가 '쫄깃한 떡볶이'와 '부드러운 라면' 중 고민될 때",
    a1: { text: "즉흥적으로 쫄깃한 떡볶이를 선택한다", tags: ["P"] },
    a2: { text: "계획적으로 쫄깃한 떡볶이를 선택한다", tags: ["J"] },
  },
  {
    id: 4,
    q: "쫄깃한 음식을 선택할 때 친구가 '왜 그거 골랐어?'라고 물어볼 때",
    a1: { text: "인내심과 만족감 때문에 선택했다고 말한다", tags: ["F"] },
    a2: { text: "질감과 효율이 좋아서 선택했다고 말한다", tags: ["T"] },
  },
  {
    id: 5,
    q: "쫄깃한 음식을 먹다가 쫄깃해서 씹는 게 너무 힘들고 턱이 아플 때",
    a1: { text: "빨리 먹어서 끝낸다", tags: ["E"] },
    a2: { text: "천천히 쫄깃한 맛을 즐긴다", tags: ["I"] },
  },
  {
    id: 6,
    q: "쫄깃한 음식을 먹기 전에 언제 먹을지 계획할 때",
    a1: { text: "즉시 먹고 즐긴다", tags: ["P"] },
    a2: { text: "미리 언제 어떻게 먹을지 계획한다", tags: ["J"] },
  },
  {
    id: 7,
    q: "쫄깃한 음식에 대한 리뷰나 정보를 확인할 때",
    a1: { text: "대략적으로 확인하고 바로 선택한다", tags: ["N"] },
    a2: { text: "정확히 확인하고 신중하게 선택한다", tags: ["S"] },
  },
  {
    id: 8,
    q: "쫄깃한 음식을 먹을 때 친구가 와서 '나도 먹고 싶어!'라고 할 때",
    a1: { text: "즉시 함께 먹는다", tags: ["E"] },
    a2: { text: "혼자만 즐긴다", tags: ["I"] },
  },
  {
    id: 9,
    q: "쫄깃한 음식을 선택할 때 선택 기준을 정할 때",
    a1: { text: "감각과 직감으로 선택한다", tags: ["F"] },
    a2: { text: "논리와 정보로 선택한다", tags: ["T"] },
  },
  {
    id: 10,
    q: "쫄깃한 음식을 먹기 전에 메뉴를 미리 정해둘 때",
    a1: { text: "미리 언제 무엇을 먹을지 계획한다", tags: ["J"] },
    a2: { text: "그때그때 즉흥적으로 선택한다", tags: ["P"] },
  },
  {
    id: 11,
    q: "쫄깃한 음식을 선택할 때 과거에 먹었던 경험을 활용할 때",
    a1: { text: "과거에 먹었던 쫄깃한 음식을 다시 선택한다", tags: ["S"] },
    a2: { text: "새로운 쫄깃한 음식을 시도해본다", tags: ["N"] },
  },
  {
    id: 12,
    q: "쫄깃한 음식을 먹고 나서 다음에 무엇을 먹을지 생각할 때",
    a1: { text: "즉시 다음 음식을 찾는다", tags: ["E"] },
    a2: { text: "여유롭게 쫄깃한 맛을 즐긴다", tags: ["I"] },
  },
]

export default function FoodChewyTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/food-chewy/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/food-chewy/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("food-chewy")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("food-chewy", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
