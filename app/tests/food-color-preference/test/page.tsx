...
const questions = [
  {
    id: 1,
    q: "음식 색상 선택 빈도는?",
    a1: { text: "항상 밝은 색상 선택", tags: ["E"] },
    a2: { text: "가끔 밝은 색상 선택", tags: ["I"] },
  },
  {
    id: 2,
    q: "음식 색상 강도 선호는?",
    a1: { text: "아주 선명한 색상 선호", tags: ["N"] },
    a2: { text: "적당한 색상 선호", tags: ["S"] },
  },
  {
    id: 3,
    q: "음식 색상 선택 방식은?",
  ...
  },
  {
    id: 4,
    q: "음식 색상 선택 이유는?",
    a1: { text: "미적 감각과 아름다움", tags: ["F"] },
    a2: { text: "효율과 균형", tags: ["T"] },
  },
  {
    id: 5,
    q: "음식 색상 선택 속도는?",
...
export default function FoodColorPreferenceTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/food-color-preference/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/food-color-preference/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("food-color-preference")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("food-color-preference", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
        ...
        <Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur">
          <CardContent className="p-8">
            <div className="text-center mb-8">
              <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-gradient-to-br from-pink-400 via-purple-400 to-blue-400 flex items-center justify-center shadow-lg">
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pink-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-pink-500 bg-gradient-to-r from-pink-50 via-purple-50 to-blue-50 dark:from-pink-950 dark:via-purple-950 dark:to-blue-950 shadow-lg"
