...
const questions = [
  {
    id: 1,
    q: "바삭한 음식 선택 빈도는?",
    a1: { text: "항상 바삭한 음식 선택", tags: ["E"] },
    a2: { text: "가끔 바삭한 음식 선택", tags: ["I"] },
  },
  {
    id: 2,
    q: "바삭함 강도 선호는?",
    a1: { text: "아주 바삭한 맛 선호", tags: ["N"] },
    a2: { text: "적당한 바삭함 선호", tags: ["S"] },
  },
  {
    id: 3,
    q: "바삭한 음식 선택 방식은?",
...
export default function FoodCrispyTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/food-crispy/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/food-crispy/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("food-crispy")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("food-crispy", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
