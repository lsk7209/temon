...
const questions = [
  {
    id: 1,
    q: "아삭한 음식 선택 빈도는?",
    a1: { text: "항상 아삭한 음식 선택", tags: ["E"] },
    a2: { text: "가끔 아삭한 음식 선택", tags: ["I"] },
  },
  {
    id: 2,
    q: "아삭함 강도 선호는?",
    a1: { text: "아주 아삭한 맛 선호", tags: ["N"] },
    a2: { text: "적당한 아삭함 선호", tags: ["S"] },
  },
  {
    id: 3,
    q: "아삭한 음식 선택 방식은?",
  ...
  },
  {
    id: 4,
    q: "아삭함 선택 이유는?",
    a1: { text: "신선함과 만족감", tags: ["F"] },
    a2: { text: "질감과 효율", tags: ["T"] },
  },
  {
    id: 5,
    q: "아삭한 음식 먹는 속도는?",
...
export default function FoodCrunchyTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/food-crunchy/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/food-crunchy/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("food-crunchy")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("food-crunchy", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
        ...
        <Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur">
          <CardContent className="p-8">
            <div className="text-center mb-8">
              <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-gradient-to-br from-green-400 to-teal-500 flex items-center justify-center shadow-lg">
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-green-500 bg-gradient-to-r from-green-50 to-teal-50 dark:from-green-950 dark:to-teal-950 shadow-lg"
