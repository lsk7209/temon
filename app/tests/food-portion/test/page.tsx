...
const questions = [
  {
    id: 1,
    q: "음식 양 선택 빈도는?",
    a1: { text: "항상 많은 양 선택", tags: ["E"] },
    a2: { text: "가끔 많은 양 선택", tags: ["I"] },
  },
  {
    id: 2,
    q: "음식 양 강도 선호는?",
    a1: { text: "아주 많은 양 선호", tags: ["N"] },
    a2: { text: "적당한 양 선호", tags: ["S"] },
  },
  {
    id: 3,
    q: "음식 양 선택 방식은?",
...
export default function FoodPortionTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/food-portion/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/food-portion/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("food-portion")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("food-portion", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
