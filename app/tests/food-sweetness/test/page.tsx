...
const questions = [
  {
    id: 1,
    q: "단 음식 선택 빈도는?",
    a1: { text: "항상 단 음식 선택", tags: ["E"] },
    a2: { text: "가끔 단 음식 선택", tags: ["I"] },
  },
  {
    id: 2,
    q: "단맛 강도 선호는?",
    a1: { text: "아주 달콤한 맛 선호", tags: ["N"] },
    a2: { text: "적당한 단맛 선호", tags: ["S"] },
  },
  {
    id: 3,
    q: "단 음식 선택 방식은?",
  ...
  },
  {
    id: 4,
    q: "단맛 선택 이유는?",
    a1: { text: "기분 전환과 즐거움", tags: ["F"] },
    a2: { text: "에너지와 효율", tags: ["T"] },
  },
  {
    id: 5,
    q: "단 음식 먹는 속도는?",
...
export default function FoodSweetnessTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/food-sweetness/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/food-sweetness/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("food-sweetness")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("food-sweetness", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
