...
const questions = [
  {
    id: 1,
    q: "음식 온도 선택 빈도는?",
    a1: { text: "항상 뜨거운 음식 선택", tags: ["E"] },
    a2: { text: "가끔 뜨거운 음식 선택", tags: ["I"] },
  },
  {
    id: 2,
    q: "음식 온도 강도 선호는?",
    a1: { text: "아주 뜨거운 온도 선호", tags: ["N"] },
    a2: { text: "적당한 온도 선호", tags: ["S"] },
  },
  {
    id: 3,
    q: "음식 온도 선택 방식은?",
...
export default function FoodTemperaturePreferenceTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/food-temperature-preference/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/food-temperature-preference/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("food-temperature-preference")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("food-temperature-preference", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
        ...
        <Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur">
          <CardContent className="p-8">
            <div className="text-center mb-8">
              <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-gradient-to-br from-red-400 via-orange-400 to-yellow-400 flex items-center justify-center shadow-lg">
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-red-500 bg-gradient-to-r from-red-50 via-orange-50 to-yellow-50 dark:from-red-950 dark:via-orange-950 dark:to-yellow-950 shadow-lg"
