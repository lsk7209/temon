...
const questions = [
  {
    id: 1,
    q: "아침 식사 시간은?",
    a1: { text: "규칙적, 정해진 시간", tags: ["J"] },
    a2: { text: "유연하게, 그때그때", tags: ["P"] },
  },
  {
    id: 2,
    q: "점심 식사 시간은?",
    a1: { text: "정해진 시간, 계획적", tags: ["J"] },
    a2: { text: "그때그때, 즉흥적", tags: ["P"] },
  },
  {
    id: 3,
    q: "저녁 식사 시간은?",
    a1: { text: "계획적, 일정대로", tags: ["J"] },
    a2: { text: "즉흥적, 상황에 맞게", tags: ["P"] },
  },
  {
    id: 4,
    q: "식사 시간 선택 기준은?",
  ...
  },
  {
    id: 12,
    q: "식사 시간 관리 방식은?",
    a1: { text: "체계적으로 관리", tags: ["J"] },
    a2: { text: "자유롭게 즐기기", tags: ["P"] },
  },
]
...
export default function FoodTimingTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/food-timing/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/food-timing/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("food-timing")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("food-timing", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
