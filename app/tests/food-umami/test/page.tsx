...
const questions = [
  {
    id: 1,
    q: "감칠맛 음식 선택 빈도는?",
    a1: { text: "항상 감칠맛 음식 선택", tags: ["E"] },
    a2: { text: "가끔 감칠맛 음식 선택", tags: ["I"] },
  },
  {
    id: 2,
    q: "감칠맛 강도 선호는?",
    a1: { text: "아주 진한 감칠맛 선호", tags: ["N"] },
    a2: { text: "적당한 감칠맛 선호", tags: ["S"] },
  },
  {
    id: 3,
    q: "감칠맛 음식 선택 방식은?",
  ...
  },
  {
    id: 4,
    q: "감칠맛 선택 이유는?",
    a1: { text: "깊이와 풍부함", tags: ["F"] },
    a2: { text: "영양과 효율", tags: ["T"] },
  },
  {
    id: 5,
    q: "감칠맛 음식 먹는 속도는?",
...
export default function FoodUmamiTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/food-umami/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/food-umami/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("food-umami")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("food-umami", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
