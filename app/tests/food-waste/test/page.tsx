...
const questions = [
  {
    id: 1,
    q: "음식 남기면 어떻게 하나요?",
    a1: { text: "보관, 재활용", tags: ["J"] },
    a2: { text: "버리기, 즉시 처리", tags: ["P"] },
  },
  {
    id: 2,
    q: "낭비 대처 방식은?",
    a1: { text: "계획적, 미리 예방", tags: ["J"] },
    a2: { text: "즉흥적, 그때그때", tags: ["P"] },
  },
  {
    id: 3,
    q: "낭비 대처 이유는?",
    a1: { text: "감성, 환경", tags: ["F"] },
...
export default function FoodWasteTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/food-waste/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/food-waste/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("food-waste")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("food-waste", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
