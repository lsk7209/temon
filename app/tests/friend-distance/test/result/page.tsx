...
import { FRIEND_DISTANCE_RESULTS } from "@/lib/data/friend-distance-results";

function ResultContent() {
  const searchParams = useSearchParams();
  const mbti = (searchParams.get("type") || "ISTJ").toUpperCase();
  const result = FRIEND_DISTANCE_RESULTS[mbti] || FRIEND_DISTANCE_RESULTS.ISTJ;

  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-purple-50 to-pink-50 py-10 px-4">
      <div className="max-w-2xl mx-auto space-y-6">
        <Card className="border-0 shadow-xl bg-white/90 backdrop-blur">
          <CardHeader className="text-center space-y-3">
            <Badge className="mx-auto bg-gradient-to-r from-violet-500 to-pink-500 text-white">
              {result.mbti}
            </Badge>
          ...
          <CardContent className="space-y-6">
            ...
            <section>
              <h3 className="font-semibold mb-2">💖 우정 충전 팁</h3>
              <ul className="list-disc pl-5 space-y-1 text-gray-700">
            ...
            <section className="grid grid-cols-2 gap-4 pt-4 border-t">
              <div>
                <p className="text-sm font-semibold text-gray-500 mb-1">찰떡 친구</p>
                <p className="text-lg font-bold text-violet-600">{result.recommend.join(", ")}</p>
              </div>
              <div>
                <p className="text-sm font-semibold text-gray-500 mb-1">피곤한 친구</p>
        ...
        <ShareButtons
          ...
          testPath="/tests/friend-distance"
          resultType={result.mbti}
          title={`나의 친구 거리 유형: ${result.name}`}
          description={result.summary}
        />
        <div className="flex gap-3 justify-center pt-4">
          <Button asChild variant="outline">
            <Link href="/tests/friend-distance"><RotateCcw className="w-4 h-4 mr-2" />다시 테스트</Link>
          </Button>
