...
export default function LunchPreferenceTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/lunch-preference/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/lunch-preference/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("lunch-preference")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("lunch-preference", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
