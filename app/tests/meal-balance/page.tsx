...
const shortDescription = "식사 균형으로 알아보는 나의 성격 유형 테스트"
// Full description for Google/AI
const fullDescription = "식사 균형 테스트로 알아보는 나의 성격! 12개의 질문으로 16가지 유형 중 당신은 어떤 유형일까요? 재미있는 식사 균형 테스트를 지금 바로 무료로 시작해보세요."
...
export const metadata: Metadata = generateQuizMetadata({
  ...
  shortDescription,
  fullDescription,
  keywords: "식사 균형 테스트, 성격 테스트, MBTI, 심리테스트, 무료 테스트",
  canonical: "/tests/meal-balance",
  questionCount: 12,
  duration: "PT3M",
})

const faqs = [
  ...getTopicQuizFAQs("식사 균형 테스트"),
]

export default function MealBalancePage() {
  ...
  return (
    <>
      ...
      <JsonLd id="meal-balance-quiz-schema" data={schemas.quiz} />
      <JsonLd id="meal-balance-breadcrumb-schema" data={schemas.breadcrumb} />
      {schemas.faq && <JsonLd id="meal-balance-faq-schema" data={schemas.faq} />}
          ...
          <CardContent className="p-8 md:p-12 text-center space-y-8">
            ...
            <div className="pt-8">
              <Button asChild size="lg" className="bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 hover:from-emerald-600 hover:via-teal-600 hover:to-cyan-600 text-white text-lg px-8 py-6">
                <Link href="/tests/meal-balance/test">테스트 시작하기</Link>
              </Button>
            </div>
            ...
            <div className="pt-8 text-left space-y-4">
              ...
              <p className="text-muted-foreground leading-relaxed">
                식사 균형과 패턴을 통해 당신의 성격 유형을 분석합니다. 
                식사 균형은 계획성, 즉흥성, 규칙성, 유연성 등 다양한 성격 특성을 반영합니다.
              </p>
            </div>
          </CardContent>
        </Card>

        <TestExpandedIntro testId="meal-balance" />
      
        <div className="mt-12">
          <AutoGscLandingBoost testId="meal-balance" />

          <AnswerEngineSection quizTitle="Meal Balance Test" />
        </div>

        <div className="mt-12">
          <LandingConversionSection quizTitle="Meal Balance Test" />
        </div>

        <div className="mt-12">
          <RelatedTestsSection testId="meal-balance" />
        </div>

        {/* FAQ Section for AI Bot Optimization */}
        <section className="mt-12 mb-8">
          <FAQSection faqs={faqs} title="식사 균형 테스트 자주 묻는 질문" />
        </section>
</main>
    </div>
    </>
