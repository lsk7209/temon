...
const questions = [
  {
    id: 1,
    q: "식사 시간 길이는?",
    a1: { text: "오래 걸리는 식사 선호", tags: ["E"] },
    a2: { text: "짧은 식사 선호", tags: ["I"] },
  },
  {
    id: 2,
    q: "식사 시간 패턴은?",
    a1: { text: "새로운 시간 패턴 시도", tags: ["N"] },
    a2: { text: "익숙한 시간 패턴 유지", tags: ["S"] },
  },
  {
    id: 3,
    q: "식사 시간 선택은?",
...
export default function MealDurationTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/meal-duration/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/meal-duration/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("meal-duration")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("meal-duration", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
        ...
        <Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur">
          <CardContent className="p-8">
            <div className="text-center mb-8">
              <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-gradient-to-br from-violet-400 via-purple-400 to-fuchsia-400 flex items-center justify-center shadow-lg">
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-violet-500 bg-gradient-to-r from-violet-50 via-purple-50 to-fuchsia-50 dark:from-violet-950 dark:via-purple-950 dark:to-fuchsia-950 shadow-lg"
