...
const questions = [
  {
    id: 1,
    q: "식사 예절 준수 빈도는?",
    a1: { text: "항상 예절 준수", tags: ["J"] },
    a2: { text: "상황에 따라 유연", tags: ["P"] },
  },
  {
    id: 2,
    q: "식사 예절 선택 방식은?",
    a1: { text: "즉흥적으로 선택", tags: ["P"] },
    a2: { text: "계획적으로 준수", tags: ["J"] },
  },
  {
    id: 3,
    q: "식사 예절 선택 기준은?",
  ...
  },
  {
    id: 6,
    q: "식사 예절 패턴은?",
    a1: { text: "새로운 예절 시도", tags: ["N"] },
    a2: { text: "익숙한 예절 유지", tags: ["S"] },
  },
  {
    id: 7,
    q: "식사 예절 이유는?",
...
export default function MealEtiquetteTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/meal-etiquette/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/meal-etiquette/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("meal-etiquette")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("meal-etiquette", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-emerald-500 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950 dark:via-teal-950 dark:to-cyan-950 shadow-lg"
