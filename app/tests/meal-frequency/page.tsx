...
const shortDescription = "식사 빈도로 알아보는 나의 성격 유형 테스트"
// Full description for Google/AI
const fullDescription = "식사 빈도 테스트로 알아보는 나의 성격! 12개의 질문으로 16가지 유형 중 당신은 어떤 유형일까요? 재미있는 식사 빈도 테스트를 지금 바로 무료로 시작해보세요."

export const metadata: Metadata = generateQuizMetadata({
  quizId: "meal-frequency",
  title: "식사 빈도 테스트",
  shortDescription,
  fullDescription,
  keywords: "식사 빈도 테스트, 성격 테스트, MBTI, 심리테스트, 무료 테스트",
  canonical: "/tests/meal-frequency",
  questionCount: 12,
  duration: "PT3M",
})

const faqs = [
  ...getTopicQuizFAQs("식사 빈도 테스트"),
]
...
export default function MealFrequencyPage() {
  ...
  return (
    <>
      ...
      <JsonLd id="meal-frequency-quiz-schema" data={schemas.quiz} />
      <JsonLd id="meal-frequency-breadcrumb-schema" data={schemas.breadcrumb} />
      {schemas.faq && <JsonLd id="meal-frequency-faq-schema" data={schemas.faq} />}
      ...
      <main className="container max-w-4xl mx-auto px-4 py-16">
        ...
        <TestExpandedIntro testId="meal-frequency" />
      
        <div className="mt-12">
          <AutoGscLandingBoost testId="meal-frequency" />

          <AnswerEngineSection quizTitle="Meal Frequency Test" />
        </div>

        <div className="mt-12">
          <LandingConversionSection quizTitle="Meal Frequency Test" />
        </div>

        <div className="mt-12">
          <RelatedTestsSection testId="meal-frequency" />
        </div>

        {/* FAQ Section for AI Bot Optimization */}
        <section className="mt-12 mb-8">
          <FAQSection faqs={faqs} title="식사 빈도 테스트 자주 묻는 질문" />
        </section>
</main>
    </div>
    </>
