...
const questions = [
  {
    id: 1,
    q: "단체 식사 빈도는?",
    a1: { text: "항상 단체 식사", tags: ["E"] },
    a2: { text: "가끔 단체 식사", tags: ["I"] },
  },
  {
    id: 2,
    q: "단체 식사 선택 방식은?",
...
export default function MealGroupTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/meal-group/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/meal-group/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("meal-group")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("meal-group", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-violet-500 bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-950 dark:via-purple-950 dark:to-fuchsia-950 shadow-lg"
