...
const questions = [
  {
    id: 1,
    q: "식사 순서는?",
    a1: { text: "고정된 순서 유지", tags: ["J"] },
    a2: { text: "매번 다른 순서", tags: ["P"] },
  },
  {
    id: 2,
    q: "식사 순서 결정은?",
...
export default function MealOrderTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/meal-order/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/meal-order/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("meal-order")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("meal-order", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
