...
export default function MealPrepTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/meal-prep/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/meal-prep/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("meal-prep")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("meal-prep", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
