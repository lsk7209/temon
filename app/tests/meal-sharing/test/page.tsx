...
const questions = [
  {
    id: 1,
    q: "음식 공유 빈도는?",
    a1: { text: "항상 공유", tags: ["E"] },
    a2: { text: "가끔 공유", tags: ["I"] },
  },
  {
    id: 2,
    q: "음식 공유 방식은?",
    a1: { text: "즉흥적으로 공유", tags: ["P"] },
    a2: { text: "체계적으로 공유", tags: ["J"] },
  },
  {
    id: 3,
    q: "음식 공유 기준은?",
  ...
  },
  {
    id: 9,
    q: "음식 공유 시간은?",
    a1: { text: "미리 공유", tags: ["J"] },
    a2: { text: "그때그때 공유", tags: ["P"] },
  },
  {
    id: 10,
    q: "음식 공유 경험 활용은?",
...
export default function MealSharingTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/meal-sharing/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/meal-sharing/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("meal-sharing")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("meal-sharing", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
        ...
        <Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur">
          <CardContent className="p-8">
            <div className="text-center mb-8">
              <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-gradient-to-br from-green-400 via-emerald-400 to-teal-400 flex items-center justify-center shadow-lg">
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-green-500 bg-gradient-to-r from-green-50 via-emerald-50 to-teal-50 dark:from-green-950 dark:via-emerald-950 dark:to-teal-950 shadow-lg"
