...
const questions = [
  {
    id: 1,
    q: "야식 섭취 빈도는?",
    a1: { text: "항상 야식 섭취", tags: ["J"] },
    a2: { text: "가끔 야식 섭취", tags: ["P"] },
  },
  {
    id: 2,
    q: "야식 선택 방식은?",
  ...
  },
  {
    id: 6,
    q: "야식 패턴은?",
    a1: { text: "새로운 종류 시도", tags: ["N"] },
    a2: { text: "익숙한 종류 유지", tags: ["S"] },
  },
  {
    id: 7,
    q: "야식 이유는?",
...
export default function MidnightSnackTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/midnight-snack/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/midnight-snack/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("midnight-snack")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("midnight-snack", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
              ...
              <button
                ...
                className={`w-full p-6 text-left rounded-2xl border-2 transition-all duration-300 transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
                  selectedChoice === currentQ.a1.tags.join(",")
                    ? "border-indigo-500 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950 dark:via-purple-950 dark:to-pink-950 shadow-lg"
