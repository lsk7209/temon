...
export default function MorningBreakfastTest() {
  ...
  useEffect(() => {
    trackTestStart("morning-breakfast")
  }, [])
  ...
  const handleChoiceSelect = async (tags: string[]) => {
    ...
    trackTestProgress("morning-breakfast", currentQuestion + 1, questions.length)
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      try {
        ...
        router.push(`/tests/morning-breakfast/test/result?type=${result}&id=${resultId}`)
      } catch (error) {
        console.error("결과 저장 실패:", error)
        router.push(`/tests/morning-breakfast/test/result?type=${result}`)
      }
    }
  }
