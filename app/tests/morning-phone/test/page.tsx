...
export default function MorningPhoneTest() {
  ...
  const handleChoiceSelect = async (tags: string[]) => {
    ...
    trackTestProgress("morning-phone", currentQuestion + 1, questions.length)
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      try {
        ...
        router.push(`/tests/morning-phone/test/result?type=${result}&id=${resultId}`)
      } catch (error) {
        console.error("결과 저장 실패:", error)
        router.push(`/tests/morning-phone/test/result?type=${result}`)
      }
    }
  }
