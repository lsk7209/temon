...
function ResultContent() {
  ...
  return <div className="min-h-screen bg-[#F7FAFC] dark:bg-gray-950 py-8"><main className="container max-w-4xl mx-auto px-4"><Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur"><CardContent className="p-8 text-center space-y-6"><h1 className="text-3xl md:text-4xl font-bold">{label} ({type})</h1><p className="text-muted-foreground">기기 분업 전략으로 보는 디지털 워크플로우 성향에서 강점을 보이는 유형입니다.</p><div className="grid md:grid-cols-2 gap-4 text-left"><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 상황 판단과 우선순위 설정</div><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 실행 후 반복 개선 능력</div><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 협업 맥락 반영 역량</div><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 결과물 완성도 관리</div></div><div className="flex flex-col md:flex-row gap-3 justify-center"><ShareButtons testId="multi-device-productivity" testPath="/tests/multi-device-productivity/test" resultType={type} resultId={resultId} title={`내 결과는 ${label} (${type})`} description="12문항으로 알아보는 성향 테스트" /><Button variant="outline" asChild><Link href="/tests/multi-device-productivity/test"><RotateCcw className="h-4 w-4 mr-2" /> 다시 테스트</Link></Button></div></CardContent></Card></main></div>
}
