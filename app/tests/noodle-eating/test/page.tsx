...
const questions = [
  ...
  },
  {
    id: 4,
    q: "면 먹는 타이밍은?",
    a1: { text: "처음부터, 계획적으로", tags: ["J"] },
    a2: { text: "그때그때, 즉흥적으로", tags: ["P"] },
  },
  {
    id: 5,
    q: "면 먹는 방법은?",
    a1: { text: "젓가락, 전통적으로", tags: ["S"] },
    a2: { text: "숟가락, 실용적으로", tags: ["N"] },
  },
  {
    id: 6,
    q: "면 먹는 이유는?",
  ...
  },
  {
    id: 9,
    q: "면 선택 기준은?",
    a1: { text: "익숙한 면, 기본 면", tags: ["S"] },
    a2: { text: "새로운 면, 특별한 면", tags: ["N"] },
  },
  {
    id: 10,
    q: "면 먹는 계획은?",
...
export default function NoodleEatingTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/noodle-eating/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/noodle-eating/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("noodle-eating")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("noodle-eating", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
