...
import { OFFICE_SNACK_HABIT_RESULTS } from "@/lib/data/office-snack-habit-results";

function ResultContent() {
  const searchParams = useSearchParams();
  const mbti = (searchParams.get("type") || "ISTJ").toUpperCase();
  const result = OFFICE_SNACK_HABIT_RESULTS[mbti] || OFFICE_SNACK_HABIT_RESULTS.ISTJ;

  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 via-amber-50 to-orange-50 py-10 px-4">
      <div className="max-w-2xl mx-auto space-y-6">
        <Card className="border-0 shadow-xl bg-white/90 backdrop-blur">
          ...
          <CardContent className="space-y-6">
            ...
            <section>
              <h3 className="font-semibold mb-2">💡 간식 루틴 팁</h3>
              <ul className="list-disc pl-5 space-y-1 text-gray-700">
        ...
        <ShareButtons
          ...
          testPath="/tests/office-snack-habit"
          resultType={result.mbti}
          title={`나의 사무실 간식 유형: ${result.name}`}
          description={result.summary}
        />

        <div className="flex gap-3 justify-center pt-4">
          <Button asChild variant="outline">
            <Link href="/tests/office-snack-habit">
