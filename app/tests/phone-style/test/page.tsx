...
import { PHONE_STYLE_QUESTIONS, type OptionTag } from "@/lib/data/phone-style-questions"
import { decideType } from "@/lib/utils/phone-style-scorer"
...
export default function PhoneStyleTest() {
  ...
  const progress = ((currentQuestion + 1) / PHONE_STYLE_QUESTIONS.length) * 100
  ...
  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("phone-style", currentQuestion + 1, PHONE_STYLE_QUESTIONS.length)
    }
  }, [currentQuestion])
  ...
  const handleChoiceSelect = async (tag: OptionTag) => {
    ...
    setTimeout(async () => {
      ...
      if (currentQuestionIndex < PHONE_STYLE_QUESTIONS.length - 1) {
  ...
  const currentQ = PHONE_STYLE_QUESTIONS[currentQuestion]
      ...
      <div className="container max-w-2xl mx-auto px-4 py-4">
        <div className="flex items-center space-x-4">
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
            {currentQuestion + 1} / {PHONE_STYLE_QUESTIONS.length}
          </span>
