...
function ResultContent() {
  ...
  return <div className="min-h-screen bg-[#F7FAFC] dark:bg-gray-950 py-8"><main className="container max-w-4xl mx-auto px-4"><Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur"><CardContent className="p-8 text-center space-y-6"><h1 className="text-3xl md:text-4xl font-bold">{label} ({type})</h1><p className="text-muted-foreground">소개 문구와 콘텐츠 인상을 설계하는 방식에서 퍼스널 브랜딩 강점이 드러나는 유형입니다.</p><div className="grid md:grid-cols-2 gap-4 text-left"><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 브랜드 메시지 명료화</div><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 타깃 시선 최적화</div><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 프로필 일관성 유지</div><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 콘텐츠 톤 설계</div></div><div className="flex flex-col md:flex-row gap-3 justify-center"><ShareButtons testId="profile-branding-type" testPath="/tests/profile-branding-type/test" resultType={type} resultId={resultId} title={`내 결과는 ${label} (${type})`} description="12문항으로 알아보는 성향 테스트" /><Button variant="outline" asChild><Link href="/tests/profile-branding-type/test"><RotateCcw className="h-4 w-4 mr-2" /> 다시 테스트</Link></Button></div></CardContent></Card></main></div>
}
