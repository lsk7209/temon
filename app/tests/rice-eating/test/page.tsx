...
const questions = [
  ...
  },
  {
    id: 4,
    q: "반찬 선택은?",
    a1: { text: "익숙한 반찬, 기본 반찬", tags: ["S"] },
    a2: { text: "새로운 반찬, 특별한 반찬", tags: ["N"] },
  },
  {
    id: 5,
    q: "밥 먹는 분위기는?",
...
export default function RiceEatingTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/rice-eating/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/rice-eating/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("rice-eating")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("rice-eating", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
