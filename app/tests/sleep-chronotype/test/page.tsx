...
import { SLEEP_CHRONOTYPE_QUESTIONS, type OptionTag } from "@/lib/data/sleep-chronotype-questions"
...
export default function SleepChronotypeTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/sleep-chronotype/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/sleep-chronotype/test/result?type=${resultType}`)
    },
  })

  const progress = ((currentQuestion + 1) / SLEEP_CHRONOTYPE_QUESTIONS.length) * 100

  // 테스트 시작 추적
  useEffect(() => {
    trackTestStart("sleep-chronotype")
  }, [])

  // 진행률 추적
  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("sleep-chronotype", currentQuestion + 1, SLEEP_CHRONOTYPE_QUESTIONS.length)
    }
  }, [currentQuestion])
  ...
  const handleChoiceSelect = async (tag: OptionTag) => {
    ...
    setTimeout(async () => {
      ...
      if (currentQuestionIndex < SLEEP_CHRONOTYPE_QUESTIONS.length - 1) {
  ...
  const currentQ = SLEEP_CHRONOTYPE_QUESTIONS[currentQuestion]
      ...
      <div className="container max-w-2xl mx-auto px-4 py-4">
        <div className="flex items-center space-x-4">
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
            {currentQuestion + 1} / {SLEEP_CHRONOTYPE_QUESTIONS.length}
          </span>
