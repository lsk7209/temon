        ...
        <Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur">
          <CardContent className="p-8 space-y-6 text-center">
            <div className="space-y-2">
              <div className="text-sm text-muted-foreground">😴 슬립맥싱 실천 유형 테스트 결과</div>
              <h1 className="text-3xl md:text-4xl font-bold">{label} ({type})</h1>
              <p className="text-muted-foreground">잠들기 전 루틴과 환경 세팅으로 보는 수면 최적화 타입에 강점이 있는 유형입니다.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-3 justify-center">
              <ShareButtons
                testId="sleepmaxxing-type"
                testPath="/tests/sleepmaxxing-type/test"
                resultType={type}
                resultId={resultId}
              ...
              <Button variant="outline" asChild>
                <Link href="/tests/sleepmaxxing-type/test"><RotateCcw className="h-4 w-4 mr-2" /> 다시 테스트</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
