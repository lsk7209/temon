...
function ResultContent() {
  ...
  return <div className="min-h-screen bg-[#F7FAFC] dark:bg-gray-950 py-8"><main className="container max-w-4xl mx-auto px-4"><Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur"><CardContent className="p-8 text-center space-y-6"><h1 className="text-3xl md:text-4xl font-bold">{label} ({type})</h1><p className="text-muted-foreground">식단 선택과 생활 루틴으로 보는 건강 관리 성향에 강점이 있는 유형입니다.</p><div className="grid md:grid-cols-2 gap-4 text-left"><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 우선순위 설정 능력</div><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 실행 후 개선 역량</div><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 맥락 기반 커뮤니케이션</div><div className="rounded-xl bg-gray-50 dark:bg-gray-900 p-4">✅ 완성도 관리 습관</div></div><div className="flex flex-col md:flex-row gap-3 justify-center"><ShareButtons testId="slow-aging-food-type" testPath="/tests/slow-aging-food-type/test" resultType={type} resultId={resultId} title={`내 결과는 ${label} (${type})`} description="12문항으로 알아보는 성향 테스트" /><Button variant="outline" asChild><Link href="/tests/slow-aging-food-type/test"><RotateCcw className="h-4 w-4 mr-2" /> 다시 테스트</Link></Button></div></CardContent></Card></main></div>
}
