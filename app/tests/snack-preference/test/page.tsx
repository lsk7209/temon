...
const questions = [
  {
    id: 1,
    q: "간식 섭취 빈도는?",
    a1: { text: "항상 간식 섭취", tags: ["J"] },
    a2: { text: "가끔 간식 섭취", tags: ["P"] },
  },
  {
    id: 2,
    q: "간식 선택 방식은?",
...
export default function SnackPreferenceTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/snack-preference/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/snack-preference/test/result?type=${resultType}`)
    },
  })
  ...
  useEffect(() => {
    trackTestStart("snack-preference")
  }, [])

  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("snack-preference", currentQuestion + 1, questions.length)
    }
  }, [currentQuestion, questions.length])
