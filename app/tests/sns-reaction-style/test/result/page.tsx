...
import { SNS_REACTION_STYLE_RESULTS } from "@/lib/data/sns-reaction-style-results";

function ResultContent() {
  const searchParams = useSearchParams();
  const mbti = (searchParams.get("type") || "ISTJ").toUpperCase();
  const result = SNS_REACTION_STYLE_RESULTS[mbti] || SNS_REACTION_STYLE_RESULTS.ISTJ;
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <Card className="border-0 shadow-xl bg-white/90 backdrop-blur">
          <CardHeader className="text-center space-y-3">
            <Badge className="mx-auto bg-gradient-to-r from-cyan-500 to-blue-500 text-white">{result.mbti}</Badge>
          ...
          <CardContent className="space-y-6">
            ...
            <section>
              <h3 className="font-semibold mb-2">💡 SNS 활용 팁</h3>
              <ul className="list-disc pl-5 space-y-1 text-gray-700">
            ...
            <section className="grid grid-cols-2 gap-4 pt-4 border-t">
              <div>
                ...
                <p className="text-lg font-bold text-cyan-600">{result.recommend.join(", ")}</p>
              </div>
              <div>
        ...
        <ShareButtons
          ...
          testPath="/tests/sns-reaction-style"
          resultType={result.mbti}
          title={`나의 SNS 반응 유형: ${result.name}`}
          description={result.summary}
        />
        <div className="flex gap-3 justify-center pt-4">
          <Button asChild variant="outline">
            <Link href="/tests/sns-reaction-style"><RotateCcw className="w-4 h-4 mr-2" />다시 테스트</Link>
          </Button>
