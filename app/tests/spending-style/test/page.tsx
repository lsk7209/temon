...
import { SPENDING_STYLE_QUESTIONS, type OptionTag } from "@/lib/data/spending-style-questions"
...
export default function SpendingStyleTest() {
  ...
  const { saveResult, isSaving } = useTestResult({
    ...
    onSuccess: (resultId, resultType) => {
      router.push(`/tests/spending-style/test/result?type=${resultType}&id=${resultId}`)
    },
    onError: (error, resultType) => {
      console.error("결과 저장 실패:", error)
      router.push(`/tests/spending-style/test/result?type=${resultType}`)
    },
  })

  const progress = ((currentQuestion + 1) / SPENDING_STYLE_QUESTIONS.length) * 100

  // 테스트 시작 추적
  useEffect(() => {
    trackTestStart("spending-style")
  }, [])

  // 진행률 추적
  useEffect(() => {
    if (currentQuestion > 0) {
      trackTestProgress("spending-style", currentQuestion + 1, SPENDING_STYLE_QUESTIONS.length)
    }
  }, [currentQuestion])
  ...
  const handleChoiceSelect = async (tag: OptionTag) => {
    ...
    setTimeout(async () => {
      ...
      if (currentQuestionIndex < SPENDING_STYLE_QUESTIONS.length - 1) {
  ...
  const currentQ = SPENDING_STYLE_QUESTIONS[currentQuestion]
      ...
      <div className="container max-w-2xl mx-auto px-4 py-4">
        <div className="flex items-center space-x-4">
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
            {currentQuestion + 1} / {SPENDING_STYLE_QUESTIONS.length}
          </span>
