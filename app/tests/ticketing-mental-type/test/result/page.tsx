        ...
        <Card className="border-0 shadow-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur">
          <CardContent className="p-8 space-y-6 text-center">
            <div className="space-y-2">
              <div className="text-sm text-muted-foreground">🎟️ 티켓팅 멘탈 유형 테스트 결과</div>
              <h1 className="text-3xl md:text-4xl font-bold">{label} ({type})</h1>
              <p className="text-muted-foreground">예매 직전 행동과 실패 대처 방식으로 보는 팬덤 행동 성향에 강점이 있는 유형입니다.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-3 justify-center">
              <ShareButtons
                testId="ticketing-mental-type"
                testPath="/tests/ticketing-mental-type/test"
                resultType={type}
                resultId={resultId}
              ...
              <Button variant="outline" asChild>
                <Link href="/tests/ticketing-mental-type/test"><RotateCcw className="h-4 w-4 mr-2" /> 다시 테스트</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
