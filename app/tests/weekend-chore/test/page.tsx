...
const questions = [
  {
    id: 1,
    q: "주말에 집안일을 할 때",
    a1: { text: "정해진 순서대로 한다", tags: ["J", "S"] },
    a2: { text: "우선순위에 따라 한다", tags: ["P", "N"] },
  },
  {
    id: 2,
    q: "주말에 빨래를 해야 할 때",
...
export default function WeekendChoreTest() {
  ...
  const handleChoiceSelect = async (tags: string[]) => {
    ...
    trackTestProgress("weekend-chore", currentQuestion + 1, questions.length)
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      try {
        ...
        router.push(`/tests/weekend-chore/test/result?type=${result}&id=${resultId}`)
      } catch (error) {
        console.error("결과 저장 실패:", error)
        router.push(`/tests/weekend-chore/test/result?type=${result}`)
      }
    }
  }
