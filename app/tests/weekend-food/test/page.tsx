...
export default function WeekendFoodTest() {
  ...
  const handleChoiceSelect = async (tags: string[]) => {
    ...
    trackTestProgress("weekend-food", currentQuestion + 1, questions.length)
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      try {
        ...
        router.push(`/tests/weekend-food/test/result?type=${result}&id=${resultId}`)
      } catch (error) {
        console.error("결과 저장 실패:", error)
        router.push(`/tests/weekend-food/test/result?type=${result}`)
      }
    }
  }
