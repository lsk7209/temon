...
export default function WeekendPlanningTest() {
  ...
  useEffect(() => {
    trackTestStart("weekend-planning")
  }, [])
  ...
  const handleChoiceSelect = async (tags: string[]) => {
    ...
    trackTestProgress("weekend-planning", currentQuestion + 1, questions.length)
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      try {
        ...
        router.push(`/tests/weekend-planning/test/result?type=${result}&id=${resultId}`)
      } catch (error) {
        console.error("결과 저장 실패:", error)
        router.push(`/tests/weekend-planning/test/result?type=${result}`)
      }
    }
  }
