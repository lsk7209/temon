...
import { WFH_FOCUS_TYPE_RESULTS } from "@/lib/data/wfh-focus-type-results";

function ResultContent() {
  const searchParams = useSearchParams();
  const mbti = (searchParams.get("type") || "ISTJ").toUpperCase();
  const result = WFH_FOCUS_TYPE_RESULTS[mbti] || WFH_FOCUS_TYPE_RESULTS.ISTJ;

  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-cyan-50 to-blue-50 py-10 px-4">
      <div className="max-w-2xl mx-auto space-y-6">
        <Card className="border-0 shadow-xl bg-white/90 backdrop-blur">
          <CardHeader className="text-center space-y-3">
            <Badge className="mx-auto bg-gradient-to-r from-teal-500 to-blue-500 text-white">
              {result.mbti}
            </Badge>
          ...
          <CardContent className="space-y-6">
            ...
            <section>
              <h3 className="font-semibold mb-2">🔋 집중력 개선 팁</h3>
              <ul className="list-disc pl-5 space-y-1 text-gray-700">
        ...
        <ShareButtons
          ...
          testPath="/tests/wfh-focus-type"
          resultType={result.mbti}
          title={`나의 재택근무 집중 유형: ${result.name}`}
          description={result.summary}
        />

        <div className="flex gap-3 justify-center pt-4">
          <Button asChild variant="outline">
            <Link href="/tests/wfh-focus-type">
