...
import { WORKDAY_ENERGY_RESULTS } from "@/lib/data/workday-energy-results";

function ResultContent() {
  const searchParams = useSearchParams();
  const mbti = (searchParams.get("type") || "ISTJ").toUpperCase();
  const result = WORKDAY_ENERGY_RESULTS[mbti] || WORKDAY_ENERGY_RESULTS.ISTJ;

  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50 py-10 px-4">
      <div className="max-w-2xl mx-auto space-y-6">
        <Card className="border-0 shadow-xl bg-white/90 backdrop-blur">
          <CardHeader className="text-center space-y-3">
            <Badge className="mx-auto bg-gradient-to-r from-amber-500 to-rose-500 text-white">
              {result.mbti}
            </Badge>
        ...
        <ShareButtons
          ...
          testPath="/tests/workday-energy"
          resultType={result.mbti}
          title={`나의 업무 에너지 유형: ${result.name}`}
          description={result.summary}
        />

        <div className="flex gap-3 justify-center pt-4">
          <Button asChild variant="outline">
            <Link href="/tests/workday-energy">
