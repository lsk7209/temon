...
export function buildInterpretation(
  ...
  const first = traits[0] || "반복되는 선택 기준이 비교적 선명하게 드러납니다.";
  const second = traits[1] || "상황이 달라져도 비슷한 판단 흐름을 유지하는 편입니다.";

  return [
    `${resultName} 결과는 단순한 성격 딱지가 아니라, 선택 순간에 자주 쓰는 판단 기준을 요약한 프로필입니다.`,
    first,
    second,
...
export function buildCompareSignals(
  ...
  return [
    ...
    readableRecommendations
      ...
      :
      "가까운 유형과 비교할 때는 선택 속도, 기준의 일관성, 사람과 상황 중 무엇을 먼저 보는지가 차이를 만듭니다.",
