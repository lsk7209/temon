#!/usr/bin/env node
/**
 * 신규 퀴즈 100개 주제 검증 및 큐 등록 스크립트.
 *
 * 기본은 dry-run입니다.
 *   node scripts/add-100-quiz-topics.js
 * 실제 DB 등록:
 *   node scripts/add-100-quiz-topics.js --apply
 */
...
function collectReservedStrings() {
  ...
  return files.flatMap((relativePath) => {
    ...
    return [...quoted, ...inlineSlugs].filter((value) =>
      /테스트|style|type|habit|reaction|planning|recovery|focus|course|subscription|membership|challenge|meme|webtoon|concert|exhibition/i.test(
        value,
      ),
    );
  });
...
function validateDuplicates(topics, existingSlugs, reservedStrings) {
  const warnings = [];
  const seenSlugs = new Set();

  topics.forEach((topic) => {
    if (seenSlugs.has(topic.slug)) {
      warnings.push(`[PLAN_DUPLICATE] ${topic.slug}`);
    }
    seenSlugs.add(topic.slug);

    if (existingSlugs.includes(topic.slug)) {
      warnings.push(`[EXISTING_SLUG] ${topic.slug}`);
    }

    for (const existing of existingSlugs) {
      const { score, shared } = similarity(topic.slug, existing);
    ...
    for (const reserved of reservedStrings) {
      const slugSimilarity = similarity(topic.slug, reserved);
      const titleSimilarity = similarity(topic.title, reserved);
      const score = Math.max(slugSimilarity.score, titleSimilarity.score);
      const shared = Math.max(slugSimilarity.shared, titleSimilarity.shared);
      if (shared >= 2 && score >= THRESHOLD) {
        warnings.push(
          `[SIMILAR_RESERVED] ${topic.slug} <-> ${reserved} (${score.toFixed(2)})`,
        );
        break;
      }
    }
...
async function main() {
  ...
  console.log(`reserved strings: ${reservedStrings.length}`);
  ...
  if (shapeErrors.length || duplicateWarnings.length) {
    [...shapeErrors, ...duplicateWarnings].forEach((message) =>
      console.log(`- ${message}`),
    );
    process.exit(1);
  }
